...
class Sensor extends c9m.Sensor {
  ...
  readDiskUsage (callback) {
    childProcess.exec('df', (error, stdout, stderr) => {
      ...
      if (header.includes('512-blocks')) {
        blockSize = 512;
      } else if (header.includes('1K-blocks')) {
        blockSize = 1024;
      } else {
        return callback('Unexpected header');
      }

      const disks = lines.reduce((disks, line) => {
        const match = line.match(/^(.+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)%\s+(\d+)\s+(\d+)\s+(\d+)%\s+(.+)$/);
